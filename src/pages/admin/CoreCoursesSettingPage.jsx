...
function CoreCoursesSettingPage() {
  ...
  const handleDeleteSelected = async () => {
    if (selectedCourses.length === 0) {
      showAlert('삭제할 교과목을 선택해주세요.');
      return;
    }

    const confirmed = await showConfirm(
      `선택한 ${selectedCourses.length}개의 교과목을 삭제하시겠습니까?\n\n⚠️ 해당 과목들의 학생 체크 데이터도 함께 삭제됩니다.`
    );
    ...
    if (confirmed) {
      ...
      for (const courseId of selectedCourses) {
        try {
          const result = await deleteCoreCourse(courseId);
          if (result.success) {
            successCount++;
          } else {
            failCount++;
